...
#Install open jdk
    sudo apt-get install openjdk-11-jdk -y && sleep 60s
    echo "---------------------------------------------------------"
    echo "JDK installation completed."
    echo "---------------------------------------------------------"
#Install python pip3, python and nfs modules
    sudo apt-get -y install python3-pip && sleep 60s
    ...
    echo "---------------------------------------------------------"
#Install Maven:
    sudo apt-get install maven -y && sleep 60s
#Step-8: Install Docker:
    sudo apt-get install ca-certificates curl gnupg lsb-release -y
    sudo curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg
    echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
    sudo apt-get update -y && sleep 30s
    sudo apt-get install docker-ce docker-ce-cli containerd.io -y    
#Step-9: Install NodeJS:
    sudo apt install nodejs npm -y && sleep 50s
#Step-10: Intall Git:
    sudo apt install git-all -y && sleep 60s
#Step-11: Install Zip:
    sudo apt install zip unzip -y && sleep 60s
